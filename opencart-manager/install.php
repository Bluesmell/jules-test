...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submit_step1'])) { // Database Configuration
        ...
        $db_prefix = trim($_POST['db_prefix'] ?? 'oc_');

        // Sanitize for writing into a PHP file string. addslashes is a basic way.
        $s_db_host = addslashes($db_host);
        $s_db_name = addslashes($db_name);
        $s_db_user = addslashes($db_user);
        $s_db_pass = addslashes($db_pass);
        $s_db_prefix = addslashes($db_prefix);

        $credentials_file = __DIR__ . '/config/db_credentials.php'; // Corrected path
        $config_dir = __DIR__ . '/config/';

        if (!is_writable($config_dir)) {
            $error_message = "Error: The configuration directory ({$config_dir}) is not writable. Please check permissions.";
            $step = 2; // Stay on step 2
        } elseif (file_exists($credentials_file) && !is_writable($credentials_file)) {
            $error_message = "Error: The configuration file ({$credentials_file}) exists but is not writable. Please check permissions.";
            $step = 2; // Stay on step 2
        } else {
            $php_content_string = "<?php\n";
            $php_content_string .= "// Database credentials generated by OpenCart Manager Installer\n";
            $php_content_string .= "define('OCM_DB_HOST', '{$s_db_host}');\n";
            $php_content_string .= "define('OCM_DB_USER', '{$s_db_user}');\n";
            $php_content_string .= "define('OCM_DB_PASS', '{$s_db_pass}');\n";
            $php_content_string .= "define('OCM_DB_NAME', '{$s_db_name}');\n";
            $php_content_string .= "define('OCM_DB_PREFIX', '{$s_db_prefix}');\n";
            $php_content_string .= "?>";

            if (file_put_contents($credentials_file, $php_content_string) !== false) {
                $success_message = "Database configuration saved successfully to config/db_credentials.php.";
                // Clear session variables if any were previously set for this step
                unset($_SESSION['install_db_host']); 
                $step = 3;
            } else {
                $error_message = "Failed to write database configuration to {$credentials_file}. Please check server logs and directory permissions.";
                $step = 2; // Stay on step 2
            }
        }
